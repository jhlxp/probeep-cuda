# DeepEP-ProbeEP CUDA fork

This tree is the ProbeEP implementation derived from `../deepep-moonep`.
It keeps DeepEP's normal NVLink/RDMA dispatch/combine data plane and supports
E256/TopK8 on 2, 4, 8 or 16 eight-GPU servers (EP16/32/64/128).

| Source | Responsibility |
|---|---|
| `csrc/deep_ep.cpp::Buffer::balanced_dispatch` | one production feedback/plan/Weight/Dispatch entry |
| `csrc/moonep_runtime.hpp` | persistent A/M controller and three-slot plan/event ring |
| `csrc/probeep_topology.hpp` | runtime multi-server E256 topology contract |
| `csrc/kernels/moonep_exchange.cu` | compact `[server,expert]` histogram exchange |
| `csrc/moonep_binding.cpp` | expert-pool, feedback, weight and grad bindings |
| `csrc/kernels/probeep_controller.cu` | independent Attention/MoE `alpha=0.90` device controller |
| `csrc/kernels/probeep_plan.cu` | compute-only intents, atomic network admission, local packing and route lowering |
| `csrc/kernels/probeep_weight_transport.cu` | pair-aware IBGDA Weight transport and FP32 gradient return |
| `tests/bench_probeep_plan.py` | complete diagnostic timing including admission, local packing and lowering |

`deep_ep/buffer.py` only forwards tensors/events to the C++ extension; no
histogram scan, planning, admission, chunk scheduling or polling runs in
Python. `plan_probeep` and `probeep_controller` are diagnostic bindings and
are not the production hot path. The immutable algorithm contract is in
`../../markdown/02_ProbeEP开发验收.md`; each multi-node execution contract is
self-contained in `../../tests/Test_01_RawData1_Eval20/README.md` and
`../../tests/Test_02_RawData1_All58/README.md`.

The remaining upstream README is retained below for DeepEP's original API and
environment requirements.

---

# DeepEP (upstream documentation)

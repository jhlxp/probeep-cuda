...
import deep_ep_cpp
# noinspection PyUnresolvedReferences
from deep_ep_cpp import BalancedHandle, Config, EventHandle
...
class Buffer:
    ...
    def __init__(self, group: Optional[dist.ProcessGroup],
        ...
        self.runtime = deep_ep_cpp.Buffer(
            ...
            low_latency_mode, explicitly_destroy, balanced_mode)
    ...
    def configure_balanced(self) -> None:
        """Allocate the persistent E256 multi-server planner/transport ring."""
        self.runtime.configure_balanced()

    def reset_balanced_probe_controller(
            self, fallback_budget_bytes: int = 32 * 1024 * 1024) -> None:
        """Reset independent A/M controller rows at a sequence bootstrap."""
        self.runtime.reset_balanced_probe_controller(fallback_budget_bytes)

    def get_balanced_expert_pool_views(self) -> Tuple[torch.Tensor, ...]:
        """Return contiguous gate/up/down weight and FP32 gradient pools."""
        return tuple(self.runtime.get_balanced_expert_pool_views())

    def balanced_dispatch(self,
                          ...
                          completed_observation: Optional[Tuple[torch.Tensor, ...]] = None,
                          feedback_valid: bool = True,
                          rdma_path_bandwidth_gbps: float = 200.0,
                          controller_alpha: float = 0.90,
                          fallback_budget_bytes: int = 0,
                          expert_weight_version: int = -1):
        """Run the fused ProbeEP feedback, plan, weight and dispatch entry."""
        config = self.get_balanced_dispatch_config() if config is None else config
        observation = completed_observation or (None,) * 6
        assert len(observation) == 6
        exec_x, exec_scales, exec_weights, exec_counts, handle, event = \
            self.runtime.balanced_dispatch(
                x[0], x[1], topk_idx, topk_weights, config,
                compute_kind,
                getattr(previous_event, 'event', None), async_finish,
                *observation, feedback_valid if completed_observation else False,
                rdma_path_bandwidth_gbps, controller_alpha,
                fallback_budget_bytes, expert_weight_version)
        return (exec_x, exec_scales), exec_weights, exec_counts, handle, \
            EventOverlap(event)

    def balanced_combine(self,
                         x: torch.Tensor,
                         handle: BalancedHandle,
                         config: Optional[Config] = None,
                         previous_event: Optional[EventOverlap] = None,
                         async_finish: bool = False,
                         release_after_combine: bool = True):
        """Combine grouped-expert output through the dispatch plan handle."""
        config = self.get_balanced_combine_config() if config is None else config
        combined_x, event = self.runtime.balanced_combine(
            x, handle, config, getattr(previous_event, 'event', None),
            async_finish, release_after_combine)
    ...
    def balanced_dispatch_backward(
            ...
            previous_event: Optional[EventOverlap] = None,
            async_finish: bool = False):
        """Replay the forward plan and produce weighted grouped gradients."""
        config = self.get_balanced_dispatch_config() if config is None else config
        exec_grad_out, event = self.runtime.balanced_dispatch_backward(
            grad_out, handle, config,
            getattr(previous_event, 'event', None), async_finish)
        return exec_grad_out, EventOverlap(event)

    def balanced_combine_backward(
            self,
            exec_grad_x: torch.Tensor,
            handle: BalancedHandle,
            config: Optional[Config] = None,
            previous_event: Optional[EventOverlap] = None,
            async_finish: bool = False):
        """Return grouped expert input gradients to source-token order."""
        config = self.get_balanced_combine_config() if config is None else config
        grad_x, event = self.runtime.balanced_combine_backward(
            exec_grad_x, handle, config,
            getattr(previous_event, 'event', None), async_finish)
        return grad_x, EventOverlap(event)

    def register_balanced_expert_pools(
            self,
            home_weight_shards: List[torch.Tensor],
            replica_weight_shards: List[torch.Tensor],
            home_grad_shards: List[torch.Tensor],
            replica_grad_shards: List[torch.Tensor]) -> None:
        """Register all expert shards for batched replica copy/reduction."""
        self.runtime.register_balanced_expert_pools(
            home_weight_shards, replica_weight_shards,
            home_grad_shards, replica_grad_shards)

    def balanced_weight_sync(self,
                             handle: BalancedHandle,
                             previous_event: Optional[EventOverlap] = None,
                             async_finish: bool = False) -> EventOverlap:
        event = self.runtime.balanced_weight_sync(
            handle, getattr(previous_event, 'event', None), async_finish)
        return EventOverlap(event)

    def balanced_grad_reduce(self,
                             handle: BalancedHandle,
                             previous_event: Optional[EventOverlap] = None,
                             async_finish: bool = False) -> EventOverlap:
        event = self.runtime.balanced_grad_reduce(
            handle, getattr(previous_event, 'event', None), async_finish)
        return EventOverlap(event)

    def balanced_finish_backward(
            self,
            handle: BalancedHandle,
            previous_event: Optional[EventOverlap] = None,
            async_finish: bool = False) -> EventOverlap:
        """Release a training plan when no replica-gradient reduction follows."""
        event = self.runtime.balanced_finish_backward(
            handle, getattr(previous_event, 'event', None), async_finish)
        return EventOverlap(event)
    ...
    @staticmethod
    def get_balanced_dispatch_config() -> Config:
        """Return the fixed-shape HT dispatch configuration used by ProbeEP."""
        return Config(Buffer.balanced_num_sms, 8, 512, 16, 128)

    @staticmethod
    def get_balanced_combine_config() -> Config:
        """Return the fixed-shape HT combine configuration used by ProbeEP."""
        return Config(Buffer.balanced_num_sms, 8, 512, 16, 128)

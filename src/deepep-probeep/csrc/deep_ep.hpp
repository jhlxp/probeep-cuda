...
private:
    ...
    // Physical allocation includes the CUDA-IPC plan tail while all original
    // DeepEP transport kernels continue to receive num_nvl_bytes unchanged.
    int64_t num_nvl_allocation_bytes = 0;
    ...
    // The paired count exchange uses a symmetric tail outside the original
    // RDMA transport extent.
    int64_t num_rdma_allocation_bytes = 0;
    ...
    // Allocated once by configure_balanced(); all iteration-sized planner,
    // receive and grouped-execution tensors live in its three-slot ring.
    std::unique_ptr<moonep::BalancedRuntime> balanced_runtime;
...
public:
    ...
    void configure_balanced();

    void reset_balanced_probe_controller(int64_t fallback_budget_bytes);

    moonep::ExpertPoolViews get_balanced_expert_pool_views() const;

    std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor,
               moonep::BalancedHandle, std::optional<EventHandle>>
    balanced_dispatch(const torch::Tensor& x, const torch::Tensor& x_scales,
    ...
    balanced_combine(const torch::Tensor& x,
    ...
    void register_balanced_expert_pools(
    ...
    balanced_weight_sync(const moonep::BalancedHandle& handle,
                         std::optional<EventHandle>& previous_event,
                         bool async);

    std::optional<EventHandle>
    balanced_grad_reduce(const moonep::BalancedHandle& handle,
                         std::optional<EventHandle>& previous_event,
                         bool async);

    std::optional<EventHandle>
    balanced_finish_backward(const moonep::BalancedHandle& handle,
                             std::optional<EventHandle>& previous_event,
                             bool async);

...
template <bool kLowLatencyMode, int kNumRDMARanks, bool kBalancedMode = false>
__global__ void
notify_dispatch(const int* num_tokens_per_rank, int* moe_recv_counter_mapped, int num_ranks,
    ...
    if (sm_id == 0) {
        ...
        // Balanced transport only exchanges rank counts.  Static DeepEP also
        // exchanges expert counts because it sizes an expert-major receive
        // tensor on the host.
        auto rdma_buffer_ptr_int = static_cast<int*>(rdma_buffer_ptr);
        const int num_rdma_count_fields = NUM_MAX_NVL_PEERS + (kBalancedMode ? 1 : num_rdma_experts + 1);
        auto rdma_recv_num_tokens_mixed = SymBuffer<int>(rdma_buffer_ptr, num_rdma_count_fields, kNumRDMARanks);
        ...
        if constexpr (not kBalancedMode) {
            #pragma unroll
        ...
        if (thread_id < kNumRDMARanks)
            rdma_recv_num_tokens_mixed.send_buffer(thread_id)[num_rdma_count_fields - 1] = num_tokens_per_rdma_rank[thread_id];
        __syncthreads();
        ...
        for (int i = warp_id; i < kNumRDMARanks; i += num_warps) {
            if (i != rdma_rank) {
                nvshmemi_ibgda_put_nbi_warp<true>(reinterpret_cast<uint64_t>(rdma_recv_num_tokens_mixed.recv_buffer(rdma_rank)),
                                                ...
                                                num_rdma_count_fields * sizeof(int),
                                                translate_dst_rdma_rank<kLowLatencyMode>(i, nvl_rank), 0, lane_id, 0);
            } else {
                UNROLLED_WARP_COPY(1, lane_id, num_rdma_count_fields,
        ...
        auto nvl_reduced_num_tokens_per_expert = Buffer<int>(nvl_recv_buffer, kBalancedMode ? 0 : num_rdma_experts).advance_also(nvl_send_buffer);
        auto nvl_send_num_tokens_per_rank = AsymBuffer<int>(nvl_send_buffer, kNumRDMARanks, NUM_MAX_NVL_PEERS);
        auto nvl_send_num_tokens_per_expert = AsymBuffer<int>(nvl_send_buffer, kBalancedMode ? 0 : num_nvl_experts, NUM_MAX_NVL_PEERS);
        auto nvl_recv_num_tokens_per_rank = AsymBuffer<int>(nvl_recv_buffer, kNumRDMARanks, NUM_MAX_NVL_PEERS);
        auto nvl_recv_num_tokens_per_expert = AsymBuffer<int>(nvl_recv_buffer, kBalancedMode ? 0 : num_nvl_experts, NUM_MAX_NVL_PEERS);
        ...
        if (thread_id == 0) {
            ...
            #pragma unroll
            for (int i = 0; i < kNumRDMARanks; ++ i) {
                sum += rdma_recv_num_tokens_mixed.recv_buffer(i)[num_rdma_count_fields - 1];
...
void balanced_notify_dispatch(const int* num_tokens_per_rank,
    ...
    auto rdma_clean_meta = get_rdma_clean_meta(
        hidden_int4, num_scales, kNumTopK, kNumTopK,
    ...
    SETUP_LAUNCH_CONFIG(1 + num_rdma_ranks, kNumThreads, stream);
#define BALANCED_NOTIFY_CASE(num_rdma) { \
    LAUNCH_KERNEL(&cfg, (notify_dispatch<false, num_rdma, true>), \
                  num_tokens_per_rank, recv_count, num_ranks, \
                  num_tokens_per_rdma_rank, recv_rdma_count, \
                  nullptr, nullptr, kNumExperts, \
                  is_token_in_rank, num_tokens, num_channels, 1, \
                  rdma_clean_meta.first, rdma_clean_meta.second, \
                  nvl_clean_meta.first, nvl_clean_meta.second, \
                  rdma_channel_prefix_matrix, recv_rdma_rank_prefix_sum, \
                  gbl_channel_prefix_matrix, recv_gbl_rank_prefix_sum, \
                  ...
                  cpu_rdma_team); } break
    SWITCH_RDMA_RANKS(BALANCED_NOTIFY_CASE);
#undef BALANCED_NOTIFY_CASE
}
...
dispatch(int4* recv_x, float* recv_x_scales, int64_t* recv_topk_idx, float* recv_topk_weights, SourceMeta* recv_src_meta,
         ...
         int* recv_route_rows, float* exec_route_weight,
         const int* route_dst, int nvs) {
    enum class WarpRole {
        kRDMASender,
    ...
    auto balanced_tma_mbarrier = reinterpret_cast<uint64_t*>(tma_buffer + hidden_bytes * 2);
    uint32_t tma_phase = 0;
    if ((warp_role == WarpRole::kRDMAAndNVLForwarder or warp_role == WarpRole::kNVLReceivers) and lane_id == 0) {
        if constexpr (kBalancedMode and not kCachedMode) {
            if (warp_role == WarpRole::kNVLReceivers) {
                mbarrier_init(balanced_tma_mbarrier, 1);
                mbarrier_init(balanced_tma_mbarrier + 1, 1);
            } else {
    ...
    if (warp_role == WarpRole::kRDMASender) {
        ...
        for (token_idx = token_start_idx; token_idx < token_end_idx; ++ token_idx) {
            ...
            for (int i = lane_id; i < num_topk * num_topk_ranks; i += 32) {
                ...
                auto idx_value = kBalancedMode
                    ? ld_nc_global(route_dst + token_idx * num_topk + copy_idx)
                    : static_cast<int>(ld_nc_global(topk_idx + token_idx * num_topk + copy_idx));
                float weight_value = 0.0f;
                if constexpr (not (kBalancedMode and kCachedMode))
                    weight_value = ld_nc_global(
                        topk_weights + token_idx * num_topk + copy_idx);
        ...
        while (num_tokens_to_recv > 0) {
            ...
            int pending_exec_row = 0, pending_stage_idx = 0;
            unsigned pending_route_mask = 0;
            const float* pending_scales = nullptr;

            auto issue_balanced_hidden_stores = [&](int stage_idx,
                                                    unsigned route_mask,
                                                    int lane_exec_row) {
                auto* stage_buffer = tma_buffer + stage_idx * hidden_bytes;
                auto* stage_mbarrier = balanced_tma_mbarrier + stage_idx;
                mbarrier_wait<true>(stage_mbarrier, tma_phase, stage_idx);

                while (route_mask != 0) {
                    const int route = __ffs(route_mask) - 1;
                    const int local_exec_row = __shfl_sync(
                        0xffffffffu, lane_exec_row, route);
                    if (lane_id == 0) {
                        const auto smem_ptr = static_cast<uint32_t>(
                            __cvta_generic_to_shared(stage_buffer));
                        const auto* output = recv_x +
                            local_exec_row * hidden_int4;
                        asm volatile(
                            "cp.async.bulk.global.shared::cta.bulk_group.L2::cache_hint "
                            "[%0], [%1], %2, %3;\n"
                            :: "l"(output), "r"(smem_ptr),
                               "r"(static_cast<int>(hidden_bytes)),
                               "l"(kEvictNormal)
                            : "memory");
                    }
            ...
            auto copy_balanced_scales = [&](const float* scales,
                                            unsigned route_mask,
                                            int lane_exec_row) {
                for (int scale = lane_id; scale < num_scales; scale += 32) {
                    const auto value = ld_nc_global(scales + scale);
                    auto routes = route_mask;
                    const auto scale_lane_mask = __activemask();
                    while (routes != 0) {
                        const int route = __ffs(routes) - 1;
                        const int local_exec_row = __shfl_sync(
                            scale_lane_mask, lane_exec_row, route);
                        st_na_global(
                            recv_x_scales +
                                local_exec_row * num_scales + scale,
                            value);
                        routes &= routes - 1;
                    }
                }
            ...
            for (int chunk_idx = 0; chunk_idx < num_recv_tokens; ++ chunk_idx, -- num_tokens_to_recv) {
                ...
                if constexpr (kBalancedMode) {
                    const auto* shifted_scales = reinterpret_cast<const float*>(shifted + hidden_bytes);
                    const auto* shifted_routes = reinterpret_cast<const int*>(
                        shifted + hidden_bytes + scale_bytes + sizeof(SourceMeta));
                    const auto* shifted_weights = reinterpret_cast<const float*>(
                        shifted_routes + num_topk);

                    const int encoded_dst = lane_id < num_topk
                        ? ld_nc_global(shifted_routes + lane_id) : 0;
                    const int exec_row = encoded_dst - rank * nvs;
                    const bool is_local_route = lane_id < num_topk and
                        static_cast<unsigned>(exec_row) <
                            static_cast<unsigned>(nvs);
                    const unsigned local_route_mask = __ballot_sync(
                        0xffffffffu, is_local_route);

                    if constexpr (not kCachedMode) {
                        if (lane_id < num_topk) {
                            recv_route_rows[recv_token_idx * num_topk + lane_id] =
                                is_local_route ? exec_row : -1;
                            if (is_local_route)
                                exec_route_weight[exec_row] =
                                    ld_nc_global(shifted_weights + lane_id);
                        }
                    }
                    ...
                    if constexpr (kCachedMode) {
                        int local_exec_rows[8];
                        #pragma unroll
                        for (int route = 0; route < 8; ++route) {
                            local_exec_rows[route] = __shfl_sync(
                                0xffffffffu, exec_row, route);
                        }

                        if (lane_id == 0) {
                            tma_load_1d(tma_buffer, shifted, tma_mbarrier,
                                        hidden_bytes, false);
                            mbarrier_arrive_and_expect_tx(tma_mbarrier,
                                                         hidden_bytes);
                        }
                        __syncwarp();
                        mbarrier_wait(tma_mbarrier, tma_phase);
                        ...
                        if (lane_id == 0) {
                            #pragma unroll
                            for (int route = 0; route < 8; ++route) {
                                if (local_route_mask & (1u << route))
                                    tma_store_1d(
                                        tma_buffer,
                                        recv_x + local_exec_rows[route] * hidden_int4,
                                        hidden_bytes, false);
                            }
                        }
                        __syncwarp();
                        ...
                        // Scales are only 224 bytes on the FP8 path.  A warp
                        // load with vector fanout avoids a second TMA group and
                        // is a no-op for cached BF16 dispatch (num_scales == 0).
                        for (int scale = lane_id; scale < num_scales; scale += 32) {
                            const auto value = ld_nc_global(shifted_scales + scale);
                            #pragma unroll
                            for (int route = 0; route < 8; ++route) {
                                if (local_route_mask & (1u << route))
                                    st_na_global(
                                        recv_x_scales +
                                            local_exec_rows[route] * num_scales +
                                            scale,
                                        value);
                            }
                        }
                    } else {
                        // Resolve the current row while the previous TMA load is
                        // in flight.  Fanout copies for one token form one bulk
                        // group, so wait_group<1> safely releases the older of
                        // the two 7 KiB shared-memory stages before reuse.
                        const int stage_idx = chunk_idx & 1;
                        auto* stage_buffer = tma_buffer + stage_idx * hidden_bytes;
                        auto* stage_mbarrier = balanced_tma_mbarrier + stage_idx;

                        if (chunk_idx != 0)
                            issue_balanced_hidden_stores(
                                pending_stage_idx, pending_route_mask,
                                pending_exec_row);
                        if (chunk_idx > 1 and lane_id == 0)
                            tma_store_wait<1>();

                        if (lane_id == 0) {
                            tma_load_1d(stage_buffer, shifted, stage_mbarrier,
                                        hidden_bytes, false);
                            mbarrier_arrive_and_expect_tx(stage_mbarrier,
                                                         hidden_bytes);
                        }
                        if (chunk_idx != 0)
                            copy_balanced_scales(
                                pending_scales, pending_route_mask,
                                pending_exec_row);

                        pending_exec_row = exec_row;
                        pending_route_mask = local_route_mask;
                        pending_scales = shifted_scales;
            ...
            if constexpr (kBalancedMode and not kCachedMode) {
                issue_balanced_hidden_stores(
                    pending_stage_idx, pending_route_mask, pending_exec_row);
                copy_balanced_scales(
                    pending_scales, pending_route_mask, pending_exec_row);
                if (lane_id == 0)
                    tma_store_wait();
                __syncwarp();
            }
...
void balanced_dispatch(void* exec_x,
    ...
    SETUP_LAUNCH_CONFIG(num_channels * 2,
                        ...
                        stream);
#define BALANCED_DISPATCH_CASE(num_rdma) { \
    auto dispatch_func = dispatch<false, num_rdma, false, \
                                  kNumTMABytesPerWarp, \
                                  kNumDispatchRDMASenderWarps, \
                                  get_num_topk_rdma_ranks(num_rdma), true>; \
    SET_SHARED_MEMORY_FOR_TMA(dispatch_func); \
    LAUNCH_KERNEL(&cfg, dispatch_func, \
                  reinterpret_cast<int4*>(exec_x), exec_x_scales, \
                  nullptr, nullptr, reinterpret_cast<SourceMeta*>(recv_src_meta), \
                  reinterpret_cast<const int4*>(x), x_scales, \
                  nullptr, route_weight, \
                  send_rdma_head, send_nvl_head, \
                  ...
                  gbl_channel_prefix_matrix, recv_gbl_rank_prefix_sum, \
                  is_token_in_rank, \
                  num_tokens, hidden_int4, num_scales, kNumTopK, kNumExperts, \
                  scale_token_stride, scale_hidden_stride, \
                  rdma_buffer_ptr, \
                  ...
                  rank, num_ranks, \
                  recv_route_rows, exec_route_weight, route_dst, nvs); } break
    SWITCH_RDMA_RANKS(BALANCED_DISPATCH_CASE);
#undef BALANCED_DISPATCH_CASE
}
...
void balanced_cached_dispatch(
    ...
    // Cached notify is the collective queue reset for this replay.  Route and
    // weight fields retain the forward message layout even though cached
    // balanced dispatch only consumes the routes.
    cached_notify(hidden_int4, 0, kNumTopK, kNumTopK,
                  num_ranks, num_channels, 0, nullptr,
                  nullptr, nullptr, nullptr,
                  rdma_buffer_ptr, num_max_rdma_chunked_recv_tokens,
                  buffer_ptrs, num_max_nvl_chunked_recv_tokens,
                  barrier_signal_ptrs, rank, stream,
                  num_rdma_bytes, num_nvl_bytes, true, false);

    SETUP_LAUNCH_CONFIG(num_channels * 2,
                        (kNumDispatchRDMASenderWarps + 1 +
                         NUM_MAX_NVL_PEERS) * 32,
                        stream);
#define BALANCED_CACHED_DISPATCH_CASE(num_rdma) { \
    auto dispatch_func = dispatch<false, num_rdma, true, \
                                  kNumTMABytesPerWarp, \
                                  kNumDispatchRDMASenderWarps, \
                                  get_num_topk_rdma_ranks(num_rdma), true>; \
    SET_SHARED_MEMORY_FOR_TMA(dispatch_func); \
    LAUNCH_KERNEL(&cfg, dispatch_func, \
                  reinterpret_cast<int4*>(exec_x), nullptr, \
                  nullptr, nullptr, nullptr, \
                  reinterpret_cast<const int4*>(grad_out), nullptr, \
                  nullptr, nullptr, \
                  nullptr, nullptr, nullptr, nullptr, \
                  rdma_channel_prefix_matrix, recv_rdma_rank_prefix_sum, \
                  gbl_channel_prefix_matrix, recv_gbl_rank_prefix_sum, \
                  is_token_in_rank, \
                  num_tokens, hidden_int4, 0, kNumTopK, kNumExperts, \
                  0, 0, \
                  rdma_buffer_ptr, \
                  num_max_rdma_chunked_send_tokens, \
                  num_max_rdma_chunked_recv_tokens, \
                  ...
                  num_max_nvl_chunked_recv_tokens, \
                  rank, num_ranks, \
                  nullptr, nullptr, route_dst, nvs); } break
    SWITCH_RDMA_RANKS(BALANCED_CACHED_DISPATCH_CASE);
#undef BALANCED_CACHED_DISPATCH_CASE
}
...
template<bool kLowLatencyMode,
         ...
         bool kBalancedMode = false,
         bool kPrecombinedBalanced = false>
...
combine(int4* combined_x, float* combined_topk_weights,
        ...
        void** buffer_ptrs, int num_max_nvl_chunked_send_tokens, int num_max_nvl_chunked_recv_tokens,
        int rank, int num_ranks,
        const int* recv_route_rows, const float* exec_route_weight,
        const int* recv_count, int nvs) {
    enum class WarpRole {
        kNVLSender,
    ...
    const int num_transport_rows = kBalancedMode ? ld_nc_global(recv_count) : num_tokens;
    ...
    if (warp_role == WarpRole::kNVLSender) {
        ...
        if (lane_id < kNumRDMARanks) {
            ...
            token_end_idx = (prefix_idx == num_channels * num_ranks - 1) ? num_transport_rows : gbl_channel_prefix_matrix[prefix_idx + 1];
        }
        __syncwarp();
        ...
        while (true) {
            ...
            for (int i = 0; i < kNumRDMARanks; ++ i) {
                ...
                for (int chunk_idx = 0; chunk_idx < num_tokens_in_chunk; ++ chunk_idx, ++ token_idx) {
                    ...
                    // Load data.  Balanced execution is already expert-grouped;
                    // gather this transport row's local assignment rows and
                    // reduce directly into the NVL message staging buffer.
                    // A null weight pointer selects dispatch-backward's plain
                    // sum; forward combine supplies the saved route weights.
                    auto shifted_x_buffers = nvl_channel_x.buffer() + dst_slot_idx * num_bytes_per_token;
                    if constexpr (kBalancedMode and not kPrecombinedBalanced) {
                        if (lane_id == 0)
                            tma_store_wait();
                        __syncwarp();

                        // Fetch route metadata once with lanes 0..7 and keep a
                        // warp-uniform active mask.  The common single-route path
                        // broadcasts once; the multi-route path broadcasts routes
                        // as it consumes them instead of keeping two eight-element
                        // arrays live across the entire hidden-row loop.
                        int lane_exec_row = -1;
                        float lane_route_weight = 1.0f;
                        if (lane_id < 8) {
                            lane_exec_row = ld_nc_global(
                                recv_route_rows + token_idx * num_topk + lane_id);
                            if (lane_exec_row >= 0 and exec_route_weight != nullptr)
                                lane_route_weight = ld_nc_global(
                                    exec_route_weight + lane_exec_row);
                        }

                        const unsigned active_route_mask = __ballot_sync(
                            0xffffffff, lane_id < 8 and lane_exec_row >= 0);
                        const int num_active_routes = __popc(active_route_mask);
                        const int first_route_lane = num_active_routes == 0
                            ? 0 : __ffs(active_route_mask) - 1;
                        const int first_exec_row = __shfl_sync(
                            0xffffffff, lane_exec_row, first_route_lane);
                        const float first_route_weight = __shfl_sync(
                            0xffffffff, lane_route_weight, first_route_lane);

                        constexpr int kBF16x2PerInt4 = sizeof(int4) / sizeof(__nv_bfloat162);
                        for (int i = lane_id; i < hidden_int4; i += 32) {
                            int4 packed_sum{};
                            if (num_active_routes == 1 and exec_route_weight == nullptr) {
                                // Combine-backward has no gate weight.  A token
                                // with one local route needs no arithmetic.
                                packed_sum = ld_nc_global(
                                    x + first_exec_row * hidden_int4 + i);
                            } else if (num_active_routes == 1) {
                                // The common forward case on a rank is one
                                // local route: one vector load, four BF16x2
                                // converts/multiplies, and one vector store.
                                const int4 packed = ld_nc_global(
                                    x + first_exec_row * hidden_int4 + i);
                                const auto* packed_values =
                                    reinterpret_cast<const __nv_bfloat162*>(&packed);
                                auto* packed_sum_values =
                                    reinterpret_cast<__nv_bfloat162*>(&packed_sum);
                                const float weight = first_route_weight;
                                #pragma unroll
                                for (int pair = 0; pair < kBF16x2PerInt4; ++pair) {
                                    const float2 values = __bfloat1622float2(
                                        packed_values[pair]);
                                    packed_sum_values[pair] = __floats2bfloat162_rn(
                                        values.x * weight, values.y * weight);
                                }
                            } else {
                                float2 sums[kBF16x2PerInt4];
                                #pragma unroll
                                for (int pair = 0; pair < kBF16x2PerInt4; ++pair)
                                    sums[pair] = make_float2(0.0f, 0.0f);

                                unsigned remaining_routes = active_route_mask;
                                while (remaining_routes != 0) {
                                    const int route_lane = __ffs(remaining_routes) - 1;
                                    remaining_routes &= remaining_routes - 1;
                                    const int exec_row = __shfl_sync(
                                        0xffffffff, lane_exec_row, route_lane);
                                    const int4 packed = ld_nc_global(
                                        x + exec_row * hidden_int4 + i);
                                    const auto* packed_values =
                                        reinterpret_cast<const __nv_bfloat162*>(&packed);
                                    const float weight = exec_route_weight == nullptr
                                        ? 1.0f : __shfl_sync(
                                            0xffffffff, lane_route_weight, route_lane);
                                    #pragma unroll
                                    for (int pair = 0; pair < kBF16x2PerInt4; ++pair) {
                                        const float2 values = __bfloat1622float2(
                                            packed_values[pair]);
                                        sums[pair].x += values.x * weight;
                                        sums[pair].y += values.y * weight;
                                    }
                                }
                            ...
                            reinterpret_cast<int4*>(tma_buffer)[i] = packed_sum;
                        }
                        __syncwarp();
                    } else {
                    ...
                    if (lane_id < num_topk) {
                        *reinterpret_cast<float*>(tma_buffer + hidden_bytes + sizeof(SourceMeta) + lane_id * sizeof(float)) =
                            kBalancedMode ? 0.0f : ld_nc_global(topk_weights + token_idx * num_topk + lane_id);
                    }
        ...
        if (warp_role == WarpRole::kNVLAndRDMAForwarder) {
            ...
            for (int token_start_idx = 0; token_start_idx < num_tokens_to_combine; token_start_idx += num_max_rdma_chunked_send_tokens) {
                ...
                for (int token_idx = token_start_idx + sub_warp_id; token_idx < token_end_idx; token_idx += kNumWarpsPerForwarder) {
                    ...
                    combine_token<NUM_MAX_NVL_PEERS, false, dtype_t, NUM_MAX_NVL_PEERS, true, kNumStages, kNumTMALoadBytes>(expected_head >= 0,
                                                                                                                            ...
                                                                                                                            hidden_int4, kBalancedMode ? 0 : num_topk,
            ...
            for (int64_t token_idx = token_start_idx + warp_id; token_idx < token_end_idx; token_idx += kNumRDMAReceivers) {
                ...
                combine_token<kNumRDMARanks, true, dtype_t, kNumTopkRDMARanks, false, 2>(expected_head >= 0,
                                                                                      ...
                                                                                      kBalancedMode ? nullptr : combined_topk_weights + token_idx * num_topk,
...
void balanced_combine(void* combined_x,
    ...
    constexpr int smem_size = std::max(
        kNumTMABytesPerSenderWarp * NUM_MAX_NVL_PEERS,
        kNumTMABytesPerForwarderWarp * kNumCombineForwarderWarps);

    const int num_rdma_ranks = num_ranks / NUM_MAX_NVL_PEERS;
    const int warps_per_forwarder =
            std::max(kNumCombineForwarderWarps / num_rdma_ranks, 1);
    const int num_forwarders = num_rdma_ranks * warps_per_forwarder;
    SETUP_LAUNCH_CONFIG(num_channels * 2,
                        (num_forwarders + 1) * 32,
                        stream);
#define BALANCED_COMBINE_CASE(num_rdma) { \
    constexpr int topk_rdma = get_num_topk_rdma_ranks(num_rdma); \
    constexpr int warps_per = \
            kNumCombineForwarderWarps / num_rdma > 0 ? \
            kNumCombineForwarderWarps / num_rdma : 1; \
    constexpr int forwarders = num_rdma * warps_per; \
    constexpr int receivers = forwarders - NUM_MAX_NVL_PEERS; \
    auto combine_func = combine<false, num_rdma, nv_bfloat16, \
    ...
    LAUNCH_KERNEL(&cfg, combine_func, \
                  ...
                  reinterpret_cast<const int4*>(exec_y), nullptr, \
                  nullptr, nullptr, \
                  combined_rdma_head, combined_nvl_head, \
                  ...
                  nvs, num_combined_tokens, hidden, kNumTopK, \
                  rdma_buffer_ptr, \
                  num_max_rdma_chunked_send_tokens, \
                  num_max_rdma_chunked_recv_tokens, \
                  buffer_ptrs, \
                  ...
                  num_max_nvl_chunked_recv_tokens, \
                  rank, num_ranks, \
                  recv_route_rows, exec_route_weight, recv_count, nvs); } break
    SWITCH_RDMA_RANKS(BALANCED_COMBINE_CASE);
#undef BALANCED_COMBINE_CASE
}

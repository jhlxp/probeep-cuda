...
#include "kernels/moonep_exchange.cuh"
#include "kernels/moonep_transport.cuh"
#include "kernels/probeep_controller.cuh"
#include "kernels/probeep_histogram.cuh"
...
void Buffer::configure_balanced() {
    EP_HOST_ASSERT(available);
    EP_HOST_ASSERT(balanced_mode);
    EP_HOST_ASSERT(probeep::supported_world_size(num_ranks));
    EP_HOST_ASSERT(num_nvl_bytes > 0 and num_rdma_bytes > 0);
    EP_HOST_ASSERT(not low_latency_mode);
    EP_HOST_ASSERT(balanced_runtime);

    auto* symmetric_plan_base =
            static_cast<uint8_t*>(rdma_buffer_ptr) + num_rdma_bytes;
    balanced_runtime->configure(
            rank, num_ranks, device_id, internode::get_source_meta_bytes(),
            num_nvl_bytes,
            buffer_ptrs[nvl_rank], buffer_ptrs_gpu, barrier_signal_ptrs_gpu,
            symmetric_plan_base, comm_stream);
}

void Buffer::reset_balanced_probe_controller(int64_t fallback_budget_bytes) {
    EP_HOST_ASSERT(available);
    EP_HOST_ASSERT(balanced_mode and balanced_runtime and
                   balanced_runtime->is_configured());
    balanced_runtime->reset_probe_controller(
            fallback_budget_bytes, comm_stream);
}

moonep::ExpertPoolViews Buffer::get_balanced_expert_pool_views() const {
    EP_HOST_ASSERT(available);
    EP_HOST_ASSERT(balanced_mode);
    EP_HOST_ASSERT(num_nvl_bytes > 0 and not low_latency_mode);
    return moonep::make_expert_pool_views(
            buffer_ptrs[nvl_rank],
            num_nvl_bytes + moonep::kIpcPlanReserveBytes, device_id);
}

std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor,
           moonep::BalancedHandle, std::optional<EventHandle>>
Buffer::balanced_dispatch(const torch::Tensor& x,
                          ...
                          bool async,
                          const std::optional<torch::Tensor>& compute_ns,
                          const std::optional<torch::Tensor>& network_ns,
                          const std::optional<torch::Tensor>& dispatch_tx_bytes,
                          const std::optional<torch::Tensor>& dispatch_rx_bytes,
                          const std::optional<torch::Tensor>& migration_tx_bytes,
                          const std::optional<torch::Tensor>& migration_rx_bytes,
                          bool feedback_valid,
                          double rdma_path_bandwidth_gbps,
                          double controller_alpha,
                          int64_t fallback_budget_bytes,
                          int64_t expert_weight_version) {
#ifndef DISABLE_NVSHMEM
    EP_HOST_ASSERT(balanced_runtime and balanced_runtime->is_configured());
    EP_HOST_ASSERT(x.is_cuda() and x.is_contiguous() and
                   x.scalar_type() == torch::kFloat8_e4m3fn);
    const auto topology = probeep::make_topology(num_ranks);
    EP_HOST_ASSERT(x.dim() == 2 and x.size(1) == probeep::kHidden);
    EP_HOST_ASSERT(x_scales.is_cuda() and x_scales.dim() == 2 and
                   x_scales.scalar_type() == torch::kFloat32);
    EP_HOST_ASSERT(x_scales.size(0) == x.size(0) and
                   x_scales.size(1) == probeep::kFp8Scales);
    EP_HOST_ASSERT(topk_idx.is_cuda() and topk_idx.is_contiguous() and
                   topk_idx.scalar_type() == torch::kInt64 and
                   topk_idx.dim() == 2 and topk_idx.size(0) == x.size(0) and
                   topk_idx.size(1) == probeep::kTopK);
    EP_HOST_ASSERT(topk_weights.is_cuda() and topk_weights.is_contiguous() and
                   topk_weights.scalar_type() == torch::kFloat32 and
                   topk_weights.sizes() == topk_idx.sizes());
    EP_HOST_ASSERT(x.size(0) <= probeep::kMaxTokensPerRank);
    EP_HOST_ASSERT(config.num_sms == probeep::kNumSms);
    EP_HOST_ASSERT(compute_kind == 0 || compute_kind == 1);
    EP_HOST_ASSERT(expert_weight_version >= -1);
    ...
    const bool has_feedback = compute_ns.has_value();
    EP_HOST_ASSERT(network_ns.has_value() == has_feedback and
                   dispatch_tx_bytes.has_value() == has_feedback and
                   dispatch_rx_bytes.has_value() == has_feedback and
                   migration_tx_bytes.has_value() == has_feedback and
                   migration_rx_bytes.has_value() == has_feedback);
    if (has_feedback) {
        balanced_runtime->update_probe_controller(
                compute_ns.value(), network_ns.value(),
                dispatch_tx_bytes.value(), dispatch_rx_bytes.value(),
                migration_tx_bytes.value(), migration_rx_bytes.value(),
                compute_kind, rdma_path_bandwidth_gbps, controller_alpha,
                fallback_budget_bytes, feedback_valid, comm_stream);
    }

    const int num_tokens = static_cast<int>(x.size(0));
    const int hidden_int4 = static_cast<int>(
            x.size(1) * x.element_size() / sizeof(int4));
    const int nvs = topology.num_servers * num_tokens * probeep::kTopK +
                    (probeep::kTokenPadding - 1) * topology.execution_slots;
    auto handle = balanced_runtime->acquire(num_tokens);
    auto& slot = balanced_runtime->storage(handle);
    probeep::launch_local_histogram_and_ordinal(
            topk_idx.data_ptr<int64_t>(), num_tokens,
            slot.local_histogram.data_ptr<int>(),
            slot.local_ordinal.data_ptr<int>(), comm_stream);
    moonep::publish_server_histograms(
            slot.local_histogram.data_ptr<int>(),
            balanced_runtime->symmetric_counts(), buffer_ptrs_gpu,
            barrier_signal_ptrs_gpu,
            balanced_runtime->ipc_plan_reserve_offset(), rank,
            topology.num_servers, comm_stream);
    const auto probe_workspace = probeep::ProbePlanWorkspace{
        slot.probe_global_counts.data_ptr<int>(),
        slot.local_ordinal.data_ptr<int>(),
        slot.tokens_per_expert_prefix.data_ptr<int>(),
        slot.alloc_prefix.data_ptr<int>(),
        slot.probe_alloc.data_ptr<int>(),
        slot.expert_slot.data_ptr<int>(),
        slot.route_dst.data_ptr<int>(), slot.exec_rank.data_ptr<int>(),
        slot.exec_slot.data_ptr<int>(), slot.is_token_in_rank.data_ptr<bool>(),
        slot.slot_count.data_ptr<int>(), slot.slot_begin.data_ptr<int>(),
        slot.replica_expert.data_ptr<int>(), slot.slot_expert.data_ptr<int>(),
        slot.num_tokens_per_rank.data_ptr<int>(),
        slot.num_tokens_per_rdma_rank.data_ptr<int>(),
        slot.num_tokens_per_exec_slot.data_ptr<int>(),
        slot.probe_server_load_before.data_ptr<int>(),
        slot.probe_server_load_after.data_ptr<int>(),
        slot.probe_server_padded_load_before.data_ptr<int>(),
        slot.probe_server_padded_load_after.data_ptr<int>(),
        slot.probe_assigned_tx_bytes.data_ptr<std::int64_t>(),
        slot.probe_assigned_rx_bytes.data_ptr<std::int64_t>(),
        slot.probe_dispatch_tx_bytes.data_ptr<std::int64_t>(),
        slot.probe_dispatch_rx_bytes.data_ptr<std::int64_t>(),
        slot.probe_pair_load_bytes.data_ptr<std::int64_t>(),
        slot.probe_server_expert_rows.data_ptr<int>(),
        slot.probe_compute_intents.data_ptr<int>(),
        slot.probe_migration_budget_snapshot.data_ptr<std::int64_t>(),
        slot.probe_endpoint_total_cap_bytes.data_ptr<std::int64_t>(),
        slot.probe_admitted_experts.data_ptr<int>(),
        slot.probe_deferred_experts.data_ptr<bool>(),
        slot.probe_chunk_table.data_ptr<std::int64_t>(),
        slot.probe_plan_counts.data_ptr<int>(),
    };
    const auto dispatch_wire_bytes =
            (x.size(1) * x.element_size() +
             x_scales.size(1) * x_scales.element_size() +
             internode::get_source_meta_bytes() +
             topk_idx.size(1) * (sizeof(int) + sizeof(float)) +
             sizeof(int4) - 1) / sizeof(int4) * sizeof(int4);
    probeep::launch_probeep_plan_from_ipc_counts(
            topk_idx.data_ptr<int64_t>(), buffer_ptrs_gpu,
            balanced_runtime->ipc_plan_reserve_offset(), rank, num_ranks,
            num_tokens, probeep::kTokenPadding,
            balanced_runtime->probe_migration_budget(compute_kind)
                    .data_ptr<std::int64_t>(),
            balanced_runtime->probe_controller_summary(compute_kind)
                    .data_ptr<std::int64_t>(),
            // Histogram accounting cannot observe TopK destination
            // co-occurrence.  Charge every occurrence one complete aligned
            // DeepEP wire message so dedup can overestimate but never
            // underestimate the mandatory Dispatch endpoint footprint.
            dispatch_wire_bytes,
            balanced_runtime->expert_pools_registered()
                    ? balanced_runtime->expert_weight_bytes()
                    : probeep::kDefaultDsv3ExpertWeightBytes,
            probeep::kProbeWeightChunkBytes,
            probe_workspace, comm_stream);
    // When expert pools are registered, lower Weight and Dispatch into one
    // communication-stream operation.  Every rank first completes only its
    // own RDMA-path Weight TX (while receive/materialize progresses on the
    // private stream), then
    // immediately starts that rank's DeepEP Dispatch; there is no world-wide
    // Weight-completion join.  The public weight-sync call remains idempotent
    // for correctness/debug code that invokes it after dispatch.
    if (balanced_runtime->expert_pools_registered()) {
        balanced_runtime->set_weight_version(handle, expert_weight_version);
        balanced_runtime->launch_weight_sync(handle, comm_stream);
    }
    internode::balanced_notify_dispatch(
            slot.num_tokens_per_rank.data_ptr<int>(),
            slot.num_tokens_per_rdma_rank.data_ptr<int>(),
            slot.recv_count.data_ptr<int>(),
            slot.recv_rdma_count.data_ptr<int>(),
            slot.is_token_in_rank.data_ptr<bool>(), num_tokens,
            probeep::kNumChannels, hidden_int4, probeep::kFp8Scales,
            slot.rdma_channel_prefix_matrix.data_ptr<int>(),
            slot.recv_rdma_rank_prefix_sum.data_ptr<int>(),
            slot.gbl_channel_prefix_matrix.data_ptr<int>(),
            slot.recv_gbl_rank_prefix_sum.data_ptr<int>(),
    ...
    internode::balanced_dispatch(
            slot.exec_x.data_ptr(), slot.exec_x_scales.data_ptr<float>(),
            slot.exec_weights.data_ptr<float>(),
            slot.recv_route_rows.data_ptr<int>(), slot.recv_src_meta.data_ptr(),
            x.data_ptr(), x_scales.data_ptr<float>(),
            slot.route_dst.data_ptr<int>(), topk_weights.data_ptr<float>(),
            slot.send_rdma_head.data_ptr<int>(),
            slot.send_nvl_head.data_ptr<int>(),
            slot.recv_rdma_channel_prefix_matrix.data_ptr<int>(),
            slot.recv_gbl_channel_prefix_matrix.data_ptr<int>(),
            slot.rdma_channel_prefix_matrix.data_ptr<int>(),
            slot.recv_rdma_rank_prefix_sum.data_ptr<int>(),
            slot.gbl_channel_prefix_matrix.data_ptr<int>(),
            slot.recv_gbl_rank_prefix_sum.data_ptr<int>(),
            slot.is_token_in_rank.data_ptr<bool>(), num_tokens, hidden_int4,
            probeep::kFp8Scales, static_cast<int>(x_scales.stride(0)),
            static_cast<int>(x_scales.stride(1)), nvs,
            rdma_buffer_ptr, config.num_max_rdma_chunked_send_tokens,
            config.num_max_rdma_chunked_recv_tokens, buffer_ptrs_gpu,
            config.num_max_nvl_chunked_send_tokens,
            config.num_max_nvl_chunked_recv_tokens, rank, comm_stream,
            probeep::kNumChannels, num_ranks);
    if (balanced_runtime->expert_pools_registered())
    ...
    auto exec_x = slot.exec_x.narrow(0, 0, nvs);
    auto exec_scales = slot.exec_x_scales.narrow(0, 0, nvs);
    auto exec_weights = slot.exec_weights.narrow(0, 0, nvs);
    auto local_counts = slot.slot_count.select(0, rank);
    for (auto* tensor : {&x, &x_scales, &topk_idx, &topk_weights})
    ...
    return {exec_x, exec_scales, exec_weights, local_counts, handle, event};
#else
    EP_HOST_ASSERT(false and "balanced internode dispatch requires NVSHMEM");
    return {};
#endif
}

std::tuple<torch::Tensor, std::optional<EventHandle>>
Buffer::balanced_combine(const torch::Tensor& x,
                         const moonep::BalancedHandle& handle,
                         const Config& config,
                         std::optional<EventHandle>& previous_event,
                         bool async, bool release_after_combine) {
#ifndef DISABLE_NVSHMEM
    EP_HOST_ASSERT(balanced_runtime and balanced_runtime->is_configured());
    const auto topology = probeep::make_topology(num_ranks);
    EP_HOST_ASSERT(x.is_cuda() and x.is_contiguous() and
                   x.scalar_type() == torch::kBFloat16 and
                   x.dim() == 2 and x.size(1) == probeep::kHidden);
    EP_HOST_ASSERT(x.size(0) >= topology.num_servers * handle.num_tokens() *
                                      probeep::kTopK +
                                  (probeep::kTokenPadding - 1) *
                                      topology.execution_slots);
    EP_HOST_ASSERT(config.num_sms == probeep::kNumSms);
    ...
    auto& slot = balanced_runtime->storage(handle);
    const int num_tokens = handle.num_tokens();
    const int nvs = topology.num_servers * num_tokens * probeep::kTopK +
                    (probeep::kTokenPadding - 1) * topology.execution_slots;
    const int hidden_int4 = static_cast<int>(
    ...
    internode::cached_notify(
            ...
            slot.recv_rdma_channel_prefix_matrix.data_ptr<int>(),
            slot.recv_rdma_rank_prefix_sum.data_ptr<int>(),
            slot.send_nvl_head.data_ptr<int>(), rdma_buffer_ptr,
            config.num_max_rdma_chunked_recv_tokens, buffer_ptrs_gpu,
            config.num_max_nvl_chunked_recv_tokens,
            barrier_signal_ptrs_gpu, rank, comm_stream,
            config.get_rdma_buffer_size_hint(
                    probeep::kHidden * sizeof(nv_bfloat16), num_ranks),
    ...
    internode::balanced_combine(
            ...
            slot.recv_count.data_ptr<int>(),
            slot.send_rdma_head.data_ptr<int>(),
            slot.send_nvl_head.data_ptr<int>(), slot.recv_src_meta.data_ptr(),
            slot.recv_rdma_channel_prefix_matrix.data_ptr<int>(),
            slot.recv_rdma_rank_prefix_sum.data_ptr<int>(),
            slot.recv_gbl_channel_prefix_matrix.data_ptr<int>(),
            num_tokens, probeep::kHidden, nvs, rdma_buffer_ptr,
    ...
    handle.finish(comm_stream, not release_after_combine);
    std::optional<EventHandle> event;
    if (async)
        event = EventHandle(comm_stream);
    else
    ...
    return {slot.combined_x.narrow(0, 0, num_tokens), event};
#else
    EP_HOST_ASSERT(false and "balanced internode combine requires NVSHMEM");
    return {};
#endif
}
...
Buffer::balanced_dispatch_backward(
    ...
    EP_HOST_ASSERT(balanced_runtime and balanced_runtime->is_configured());
    const auto topology = probeep::make_topology(num_ranks);
    EP_HOST_ASSERT(grad_out.is_cuda() and grad_out.is_contiguous() and
                   grad_out.scalar_type() == torch::kBFloat16);
    EP_HOST_ASSERT(grad_out.dim() == 2 and
                   grad_out.size(0) == handle.num_tokens() and
                   grad_out.size(1) == probeep::kHidden);
    EP_HOST_ASSERT(config.num_sms == probeep::kNumSms);
    ...
    const int hidden_int4 = static_cast<int>(
            probeep::kHidden * sizeof(nv_bfloat16) / sizeof(int4));
    internode::balanced_cached_dispatch(
            slot.exec_y.data_ptr(), grad_out.data_ptr(),
            slot.route_dst.data_ptr<int>(),
            slot.rdma_channel_prefix_matrix.data_ptr<int>(),
            slot.recv_rdma_rank_prefix_sum.data_ptr<int>(),
            slot.gbl_channel_prefix_matrix.data_ptr<int>(),
            slot.recv_gbl_rank_prefix_sum.data_ptr<int>(),
            ...
            config.num_max_nvl_chunked_recv_tokens,
            barrier_signal_ptrs_gpu, rank, comm_stream,
            probeep::kNumChannels,
            num_ranks,
            config.get_rdma_buffer_size_hint(
                    probeep::kHidden * sizeof(nv_bfloat16), num_ranks),
            num_nvl_bytes);
    internode::launch_bf16_route_weight_scale(
            slot.exec_y.data_ptr(), slot.exec_weights.data_ptr<float>(),
            nvs, probeep::kHidden, comm_stream);
    ...
    if (async)
        ...
        stream_wait(compute_stream, comm_stream);
    grad_out.record_stream(comm_stream);
    return {slot.exec_y.narrow(0, 0, nvs), event};
#else
    EP_HOST_ASSERT(false and
                   "balanced backward dispatch requires NVSHMEM");
    return {};
#endif
}
...
Buffer::balanced_combine_backward(
        const torch::Tensor& exec_grad_x,
        const moonep::BalancedHandle& handle,
        const Config& config,
        std::optional<EventHandle>& previous_event,
        bool async) {
    ...
    EP_HOST_ASSERT(balanced_runtime and balanced_runtime->is_configured());
    const auto topology = probeep::make_topology(num_ranks);
    EP_HOST_ASSERT(exec_grad_x.is_cuda() and exec_grad_x.is_contiguous() and
                   exec_grad_x.scalar_type() == torch::kBFloat16 and
                   exec_grad_x.dim() == 2 and
                   exec_grad_x.size(1) == probeep::kHidden);
    EP_HOST_ASSERT(config.num_sms == probeep::kNumSms);
    ...
    if (async)
        ...
        stream_wait(compute_stream, comm_stream);
    exec_grad_x.record_stream(comm_stream);
    return {slot.combined_x.narrow(0, 0, num_tokens), event};
#else
    EP_HOST_ASSERT(false and "balanced backward combine requires NVSHMEM");
    return {};
#endif
}
...
Buffer::Buffer(int rank, int num_ranks, int64_t num_nvl_bytes,
               ...
               bool explicitly_destroy, bool balanced_mode):
    ...
    if (balanced_mode)
        balanced_runtime = std::make_unique<moonep::BalancedRuntime>();

    // Metadata memory
    const auto balanced_nvl_tail = balanced_mode ?
            moonep::kIpcPlanReserveBytes + moonep::kExpertPoolBytes : 0;
    const auto balanced_rdma_tail = balanced_mode ?
            probeep::kProbeSymmetricBytes : 0;
    num_nvl_allocation_bytes = num_nvl_bytes > 0 ?
            num_nvl_bytes + balanced_nvl_tail : 0;
    num_rdma_allocation_bytes = num_rdma_bytes > 0 ?
            num_rdma_bytes + balanced_rdma_tail : 0;
    ...
    if (num_nvl_bytes > 0) {
        ...
        CUDA_CHECK(cudaMalloc(&buffer_ptrs[nvl_rank], num_nvl_allocation_bytes + barrier_signal_bytes + buffer_ptr_bytes + barrier_signal_ptr_bytes));
        CUDA_CHECK(cudaIpcGetMemHandle(&ipc_handles[nvl_rank], buffer_ptrs[nvl_rank]));
        buffer_ptrs_gpu = reinterpret_cast<void**>(static_cast<uint8_t*>(buffer_ptrs[nvl_rank]) + num_nvl_allocation_bytes + barrier_signal_bytes);

        // Set barrier signals
        barrier_signal_ptrs[nvl_rank] = reinterpret_cast<int*>(static_cast<uint8_t*>(buffer_ptrs[nvl_rank]) + num_nvl_allocation_bytes);
        barrier_signal_ptrs_gpu = reinterpret_cast<int**>(static_cast<uint8_t*>(buffer_ptrs[nvl_rank]) + num_nvl_allocation_bytes + barrier_signal_bytes + buffer_ptr_bytes);

        // No need to synchronize, will do a full device sync during `sync`
        if (balanced_mode)
            CUDA_CHECK(cudaMemsetAsync(
                    static_cast<uint8_t*>(buffer_ptrs[nvl_rank]) + num_nvl_bytes,
                    0, moonep::kIpcPlanReserveBytes, comm_stream));
...
void Buffer::sync(const std::vector<int> &device_ids,
    ...
    if (num_nvl_bytes > 0) {
        ...
        for (int i = 0, offset = rdma_rank * num_nvl_ranks; i < num_nvl_ranks; ++ i) {
            ...
            if (offset + i != rank) {
                ...
                barrier_signal_ptrs[i] = reinterpret_cast<int*>(static_cast<uint8_t*>(buffer_ptrs[i]) + num_nvl_allocation_bytes);
            } else {
    ...
    if (num_rdma_bytes > 0) {
        ...
        // Allocate
        rdma_buffer_ptr = internode::alloc(num_rdma_allocation_bytes, NUM_BUFFER_ALIGNMENT_BYTES);

        // Clean buffer (mainly for low-latency mode)
        CUDA_CHECK(cudaMemset(rdma_buffer_ptr, 0, num_rdma_allocation_bytes));
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("probeep_controller",
          [](const torch::Tensor& compute_ns,
             ...
             bool valid) {
              auto output = deep_ep::probeep::run_controller_cuda(
                  compute_ns, network_ns,
                  dispatch_tx_bytes, dispatch_rx_bytes,
                  migration_tx_bytes, migration_rx_bytes,
                  rdma_path_bandwidth_gbps, alpha,
                  fallback_budget_bytes, valid);
              pybind11::dict result;
              result["migration_budget_bytes"] = output.migration_budget_bytes;
              result["summary"] = output.summary;
              return result;
          },
          py::arg("compute_ns"), py::arg("network_ns"),
          py::arg("dispatch_tx_bytes"), py::arg("dispatch_rx_bytes"),
          py::arg("migration_tx_bytes"), py::arg("migration_rx_bytes"),
    ...
    m.def("plan_probeep",
          [](const torch::Tensor& topk_idx,
             ...
             std::int64_t learned_total_bytes) {
              auto plan = deep_ep::probeep::plan_probeep_cuda(
                  topk_idx, migration_budget_bytes,
                  expert_weight_bytes, weight_chunk_bytes,
                  ranks_per_server, local_experts,
                  replica_slots, token_padding, learned_total_bytes);
              pybind11::dict result;
              result["route_dst"] = plan.route_dst;
              result["exec_rank"] = plan.exec_rank;
              result["exec_slot"] = plan.exec_slot;
              result["is_token_in_rank"] = plan.is_token_in_rank;
              result["slot_count"] = plan.slot_count;
              result["slot_begin"] = plan.slot_begin;
              result["replica_expert"] = plan.replica_expert;
              result["slot_expert"] = plan.slot_expert;
              result["alloc"] = plan.alloc;
              result["num_tokens_per_rank"] = plan.num_tokens_per_rank;
              result["num_tokens_per_rdma_rank"] =
                      plan.num_tokens_per_rdma_rank;
              result["num_tokens_per_exec_expert"] =
                      plan.num_tokens_per_exec_expert;
              result["server_load_before"] = plan.server_load_before;
              result["server_load_after"] = plan.server_load_after;
              result["server_padded_load_before"] =
                      plan.server_padded_load_before;
              result["server_padded_load_after"] =
                      plan.server_padded_load_after;
              result["assigned_tx_bytes"] = plan.assigned_tx_bytes;
              result["assigned_rx_bytes"] = plan.assigned_rx_bytes;
              result["dispatch_tx_bytes"] = plan.dispatch_tx_bytes;
              result["dispatch_rx_bytes"] = plan.dispatch_rx_bytes;
              result["pair_load_bytes"] = plan.pair_load_bytes;
              result["compute_intents"] = plan.compute_intents;
              result["migration_budget_snapshot"] =
                      plan.migration_budget_snapshot;
              result["endpoint_total_cap_bytes"] =
                      plan.endpoint_total_cap_bytes;
              result["admitted_experts"] = plan.admitted_experts;
              result["deferred_experts"] = plan.deferred_experts;
              result["chunk_table"] = plan.chunk_table;
              result["plan_counts"] = plan.plan_counts;
              result["nvs"] = plan.nvs;
              return result;
          },
          py::arg("topk_idx"), py::arg("migration_budget_bytes"),
          py::arg("expert_weight_bytes") =
                  deep_ep::probeep::kDefaultDsv3ExpertWeightBytes,
          py::arg("weight_chunk_bytes") =
                  deep_ep::probeep::kProbeWeightChunkBytes,
          py::arg("ranks_per_server") = 8,
          py::arg("local_experts") = -1,
          py::arg("replica_slots") =
                  deep_ep::probeep::kReplicaSlots,
          py::arg("token_padding") = 8,
          py::arg("learned_total_bytes") = 0);
    pybind11::class_<deep_ep::EventHandle>(m, "EventHandle")
        .def(pybind11::init<>())
        .def("current_stream_wait", &deep_ep::EventHandle::current_stream_wait);

    deep_ep::moonep::bind_balanced_handle(m);
    auto buffer_class = pybind11::class_<deep_ep::Buffer>(m, "Buffer")
        .def(pybind11::init<int, int, int64_t, int64_t, bool, bool, bool>(),
             py::arg("rank"), py::arg("num_ranks"),
             py::arg("num_nvl_bytes"), py::arg("num_rdma_bytes"),
             py::arg("low_latency_mode"), py::arg("explicitly_destroy"),
             py::arg("balanced_mode") = false)
    ...
    deep_ep::moonep::bind_balanced_buffer(buffer_class);

...
LOCAL_EXPERTS = 16
REPLICA_SLOTS = 16
POOL_SLOTS = LOCAL_EXPERTS + REPLICA_SLOTS
TOPK = 8
HIDDEN = 7168
INTERMEDIATE = 2048
TOKEN_PADDING = 8
...
def _check_layout(views: tuple[torch.Tensor, ...]) -> None:
    ...
    assert POOL_BYTES == 8_455_716_864
...
def _expected_home_grad(
    domain_replica_map: torch.Tensor,
    server: int,
    global_expert: int,
    shard: int,
) -> float:
    expected = _home_grad_value(global_expert, shard)
    for peer_local_rank in range(RANKS_PER_SERVER):
        peer_global_rank = server * RANKS_PER_SERVER + peer_local_rank
        for replica_slot in range(REPLICA_SLOTS):
            if int(domain_replica_map[peer_local_rank, replica_slot]) == global_expert:
                expected = _f32_add(
                    expected,
                    _replica_grad_value(
                        peer_global_rank, replica_slot, shard
                    ),
                )
    return expected
    ...
    for shard, tensor in enumerate(grads):
        ...
        for local_expert in range(LOCAL_EXPERTS):
            ...
            expected = _expected_home_grad(
                domain_replica_map, server, global_expert, shard
            )
            _assert_constant(
...
def run_worker(
    ...
    server = rank // RANKS_PER_SERVER
    domain_replica_map = handle.replica_expert[
        server * RANKS_PER_SERVER : (server + 1) * RANKS_PER_SERVER
    ].cpu()
    local_replica_map = domain_replica_map[rank % RANKS_PER_SERVER]
    assert torch.all(
        (domain_replica_map == -1)
        | (
            (domain_replica_map >= server * NUM_EXPERTS // NUM_SERVERS)
            & (domain_replica_map < (server + 1) * NUM_EXPERTS // NUM_SERVERS)
        )
    )
    if server == 0:
        assert int((domain_replica_map == 0).sum()) == 7

    _initialize_replica_grads(views[3:], local_replica_map, rank)
    ...
    _check_weight_sync(views[:3], local_replica_map, rank)
    dist.barrier(group=group)

    nvs = (
        NUM_SERVERS * args.num_tokens * TOPK
        + (TOKEN_PADDING - 1) * POOL_SLOTS
    )
    ...
    _check_grad_reduce(
        views[3:], domain_replica_map, local_replica_map, rank
    )
    dist.barrier(group=group)

    if rank == 0:
        print(
            "[balanced expert I/O] fixed IPC layout, owner-push weights, "
            "ordered owner-pull FP32 grads, and replica clearing passed",
            flush=True,
        )
    buffer.destroy()
    dist.barrier()

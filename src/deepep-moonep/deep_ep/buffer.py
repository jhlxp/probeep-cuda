...
class Buffer:
    ...
    balanced_topk: int = 8
    balanced_execution_slots: int = 32
    ...
    def configure_balanced(self) -> None:
        """Allocate the persistent EP16 planner/transport ring once."""
    ...
    def balanced_dispatch(self,
        ...
        """Plan and dispatch directly into the fixed grouped-expert buffer."""
        config = self.get_balanced_dispatch_config() if config is None else config
        num_tokens = topk_idx.size(0)
        num_servers = self.group_size // self.balanced_ranks_per_server
        nvs = (num_servers * num_tokens * self.balanced_topk +
               (self.balanced_token_padding - 1) * self.balanced_execution_slots)
        ...
        return (exec_x[:nvs], exec_scales[:nvs]), exec_weights[:nvs], \
            exec_counts, handle, EventOverlap(event)
    ...
    def get_balanced_dispatch_config() -> Config:
        """Return the fixed EP16 dispatch configuration used by ProbeEP."""
        return Config(Buffer.balanced_num_sms, 8, 512, 16, 128)

    @staticmethod
    def get_balanced_combine_config() -> Config:
        """Return the fixed EP16 combine configuration used by ProbeEP."""
        return Config(Buffer.balanced_num_sms, 8, 512, 16, 128)

...
void balanced_notify_dispatch(const int* num_tokens_per_rank,
    ...
    auto rdma_clean_meta = get_rdma_clean_meta(
        ...
        kNumRDMARanks, num_max_rdma_chunked_recv_tokens, num_channels);
    ...
    SETUP_LAUNCH_CONFIG(1 + kNumRDMARanks, kNumThreads, stream);
    LAUNCH_KERNEL(&cfg, (notify_dispatch<false, kNumRDMARanks, true>),
                  num_tokens_per_rank, recv_count, kNumRanks,
...
void balanced_dispatch(void* exec_x,
    ...
    constexpr int kNumTMABytesPerWarp = 16384;
    constexpr int smem_size = kNumTMABytesPerWarp * NUM_MAX_NVL_PEERS;
    auto dispatch_func = dispatch<false, kNumRDMARanks, false,
                                  kNumTMABytesPerWarp,
                                  kNumDispatchRDMASenderWarps,
                                  get_num_topk_rdma_ranks(kNumRDMARanks),
                                  true>;
    ...
    LAUNCH_KERNEL(&cfg, dispatch_func,
                  ...
                  rank, kNumRanks,
                  recv_route_rows, exec_route_weight, route_dst, nvs);
}
...
void balanced_cached_dispatch(
    ...
    cached_notify(hidden_int4, 0, kNumTopK, kNumTopK,
                  kNumRanks, num_channels, 0, nullptr,
    ...
    auto dispatch_func = dispatch<false, kNumRDMARanks, true,
                                  kNumTMABytesPerWarp,
                                  kNumDispatchRDMASenderWarps,
                                  get_num_topk_rdma_ranks(kNumRDMARanks),
                                  true>;
...
void balanced_combine(void* combined_x,
    ...
    constexpr int kNumTMABytesPerSenderWarp = 16384;
    constexpr int kNumTMABytesPerForwarderWarp = 9248;
    constexpr int kNumTopkRDMARanks = get_num_topk_rdma_ranks(kNumRDMARanks);
    constexpr int kNumWarpsPerForwarder = kNumCombineForwarderWarps / kNumRDMARanks;
    constexpr int kNumForwarders = kNumRDMARanks * kNumWarpsPerForwarder;
    ...
    auto combine_func = combine<false, kNumRDMARanks, nv_bfloat16,
    ...
    LAUNCH_KERNEL(&cfg, combine_func,
                  ...
                  num_max_nvl_chunked_recv_tokens,
                  rank, kNumRanks,
                  recv_route_rows, exec_route_weight, recv_count, nvs);
}

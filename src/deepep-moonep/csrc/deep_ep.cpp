...
#include "kernels/moonep_plan.cuh"
...
void Buffer::configure_balanced() {
    ...
    EP_HOST_ASSERT(num_ranks == moonep::kWorldSize);
    ...
    balanced_runtime->configure(
            rank, device_id, internode::get_source_meta_bytes(), num_nvl_bytes,
...
Buffer::balanced_dispatch(const torch::Tensor& x,
    ...
    EP_HOST_ASSERT(x.dim() == 2 and x.size(1) == moonep::kHidden);
    ...
    EP_HOST_ASSERT(x_scales.size(0) == x.size(0) and
                   x_scales.size(1) == moonep::kFp8Scales);
    ...
    EP_HOST_ASSERT(x.size(0) <= moonep::kMaxTokensPerRank);
    EP_HOST_ASSERT(config.num_sms == moonep::kNumSms);
    ...
    const int nvs = moonep::kNumServers * num_tokens * moonep::kTopK +
                    (moonep::kTokenPadding - 1) * moonep::kExecutionSlots;
    auto handle = balanced_runtime->acquire(num_tokens);
    auto& slot = balanced_runtime->storage(handle);
    const auto workspace = moonep::ServerLocalPlanWorkspace{
    ...
    moonep::launch_local_histogram_and_ordinal(
            topk_idx.data_ptr<int64_t>(), num_tokens,
            slot.local_histogram.data_ptr<int>(),
            slot.local_ordinal.data_ptr<int>(), comm_stream);
    moonep::publish_paired_counts(
            slot.local_histogram.data_ptr<int>(),
            balanced_runtime->symmetric_counts(), buffer_ptrs_gpu,
            barrier_signal_ptrs_gpu,
            balanced_runtime->ipc_plan_reserve_offset(), rank, comm_stream);
    moonep::launch_server_local_plan_from_ipc_counts(
            topk_idx.data_ptr<int64_t>(), buffer_ptrs_gpu,
            balanced_runtime->ipc_plan_reserve_offset(),
            rank, num_tokens, moonep::kTokenPadding, workspace,
            slot.route_dst.data_ptr<int>(), slot.exec_rank.data_ptr<int>(),
            slot.exec_slot.data_ptr<int>(),
            slot.is_token_in_rank.data_ptr<bool>(),
            slot.slot_count.data_ptr<int>(), slot.slot_begin.data_ptr<int>(),
            slot.replica_expert.data_ptr<int>(),
            slot.slot_expert.data_ptr<int>(),
    ...
    internode::balanced_notify_dispatch(
            ...
            moonep::kNumChannels, hidden_int4, moonep::kFp8Scales,
    ...
    internode::balanced_dispatch(
            ...
            moonep::kFp8Scales, static_cast<int>(x_scales.stride(0)),
...
Buffer::balanced_combine(const torch::Tensor& x,
    ...
    EP_HOST_ASSERT(x.is_cuda() and x.is_contiguous() and
                   ...
                   x.dim() == 2 and x.size(1) == moonep::kHidden);
    EP_HOST_ASSERT(x.size(0) >= moonep::kNumServers * handle.num_tokens() *
                                      moonep::kTopK +
                                  (moonep::kTokenPadding - 1) *
                                      moonep::kExecutionSlots);
    EP_HOST_ASSERT(config.num_sms == moonep::kNumSms);
    ...
    const int nvs = moonep::kNumServers * num_tokens * moonep::kTopK +
                    (moonep::kTokenPadding - 1) * moonep::kExecutionSlots;
    const int hidden_int4 = static_cast<int>(
            moonep::kHidden * sizeof(nv_bfloat16) / sizeof(int4));
    ...
    internode::launch_precombine_bf16(
            ...
            slot.recv_count.data_ptr<int>(), moonep::kHidden, comm_stream);
    internode::cached_notify(
            hidden_int4, 0, 0, moonep::kTopK, num_ranks,
            moonep::kNumChannels, num_tokens,
            slot.send_rdma_head.data_ptr<int>(),
            slot.recv_rdma_channel_prefix_matrix.data_ptr<int>(),
            slot.recv_rdma_rank_prefix_sum.data_ptr<int>(),
            ...
            config.get_rdma_buffer_size_hint(
                    moonep::kHidden * sizeof(nv_bfloat16), num_ranks),
    ...
    internode::balanced_combine(
            ...
            slot.recv_rdma_rank_prefix_sum.data_ptr<int>(),
            slot.recv_gbl_channel_prefix_matrix.data_ptr<int>(),
            num_tokens, moonep::kHidden, nvs, rdma_buffer_ptr,
...
Buffer::balanced_dispatch_backward(
    ...
    EP_HOST_ASSERT(grad_out.dim() == 2 and
                   ...
                   grad_out.size(1) == moonep::kHidden);
    EP_HOST_ASSERT(config.num_sms == moonep::kNumSms);
    ...
    internode::launch_bf16_route_weight_scale(
            ...
            nvs, moonep::kHidden, comm_stream);
...
Buffer::balanced_combine_backward(
    ...
    EP_HOST_ASSERT(exec_grad_x.is_cuda() and exec_grad_x.is_contiguous() and
                   ...
                   exec_grad_x.size(1) == moonep::kHidden);
    EP_HOST_ASSERT(config.num_sms == moonep::kNumSms);
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("plan_server_local",
          [](const torch::Tensor& topk_idx,
             int ranks_per_server,
             int local_experts,
             int replica_slots,
             int token_padding) {
              auto plan = deep_ep::moonep::plan_server_local_cuda(
                  topk_idx, ranks_per_server, local_experts, replica_slots, token_padding);
              ...
              result["num_tokens_per_rdma_rank"] = plan.num_tokens_per_rdma_rank;
              result["num_tokens_per_exec_expert"] = plan.num_tokens_per_exec_expert;
              result["nvs"] = plan.nvs;
              return result;
          },
          py::arg("topk_idx"),
          py::arg("ranks_per_server") = 8,
          py::arg("local_experts") = 16,
          py::arg("replica_slots") = 16,

#!/usr/bin/env python3
"""Validate Test 01 raw evidence and the algorithm-first layer report."""
...
REPORT_NAME = "raw_data1_layers_00_19_by_algorithm_rounds_11_20_mean"
...
def validate_raw(
    ...
    if case_count != 100 or world_size != 16:
        raise ValueError(
            f"Test 01 requires 100 cases and 16 ranks, got {case_count} and {world_size}"
        )
    ...
    if any(path.exists() for path in microbatch_files):
        ...
        microbatch_groups: dict[tuple[str, ...], dict[int, dict[str, str]]] = (
            defaultdict(dict)
        )
        ...
        for key, by_microbatch in microbatch_groups.items():
            ...
            if sum(int(row["home_load"]) for row in by_microbatch.values()) != int(
                combined["home_load"]
            ):
                raise ValueError("MB0+MB1 home rows do not equal the combined rank row")
            if sum(int(row["exec_load"]) for row in by_microbatch.values()) != int(
                combined["exec_load"]
            ):
        ...
        microbatch_totals: dict[tuple[str, ...], list[int]] = defaultdict(
            lambda: [0, 0]
        )
        for row in microbatch_rows:
            key = tuple(
        ...
        expected_timeline_rows = 20 * 10 * world_size * (4 * 8 + 9)
        ...
        if require_fresh_visualization_raw:
            ...
            def before(
                ...
                predecessor: tuple[str, int, str],
                successor: tuple[str, int, str],
            ) -> bool:
                # CUDA event timestamps are serialized as float32 milliseconds.
            ...
            for invocation, intervals in intervals_by_invocation.items():
                ...
                if invocation[0] in {"deepep_moonep", "ultraep"}:
                    # These baselines expose one shared physical replica bank.
                    edges.append((e0, d1))
    ...
    if require_fresh_visualization_raw:
        ...
        expected_observations = 19 * 10 * 2 * world_size
        ...
        if len(plans) != 20 * 10 * 2:
            raise ValueError(
                f"probeep_plan_summary.jsonl: rows={len(plans)}, expected=400"
            )
        ...
        rail_keys = set()
        rail_paths_by_invocation: dict[tuple[str, ...], set[tuple[int, int]]] = (
            defaultdict(set)
        )
        for row in rail_rows:
            ...
            if dispatch_bytes != (
                int(row["dispatch_units"]) * int(row["dispatch_bytes_per_unit"])
            ):
            ...
            rail_paths_by_invocation[invocation].add(
        ...
        if any(
            paths != expected_directed_paths
            for paths in rail_paths_by_invocation.values()
        ):
        ...
        for row in rail_rows:
            ...
            vectors = probe_transport.setdefault(
                key,
                {
        ...
        for plan in plans:
            ...
            expected_tx = [
                int(value) * transfer_required
                for value in plan["assigned_tx_bytes"]
            ]
            expected_rx = [
                int(value) * transfer_required
                for value in plan["assigned_rx_bytes"]
            ]
        ...
        for row in observation_rows:
            consumer_key = (
                row["workload"],
                int(row["consumer_repeat"]),
                int(row["consumer_iteration"]),
            ...
            producer_key = (
                ...
                int(row["producer_repeat"]),
                int(row["producer_iteration"]),
                int(row["compute_kind"]),
            )
            ...
            expected = (
                transport["dispatch_tx"][rank],
                transport["dispatch_rx"][rank],
                transport["weight_tx"][rank],
                transport["weight_rx"][rank],
            )
            actual = (
                int(row["dispatch_tx_bytes"]),
                int(row["dispatch_rx_bytes"]),
                int(row["migration_tx_bytes"]),
                int(row["migration_rx_bytes"]),
            )
            if actual != expected:
                raise ValueError(
...
def validate_report(report_dir: Path, *, require_complete_raw: bool = False) -> None:
    ...
    if payload.get("layers") != list(range(20)):
        raise ValueError("Test 01 report must contain Layer 00..19 separately")
    ...
    if int(payload.get("num_ranks", 0)) != 16:
        raise ValueError("Test 01 report must contain 16 ranks")
    ...
    if not isinstance(methods, list) or {item.get("slug") for item in methods} != METHOD_SLUGS:
        raise ValueError("report must contain exactly the five Test 01 algorithms")
    for method in methods:
        ...
        if method.get("layer_pages") != expected_layer_pages:
            raise ValueError(f"{slug}: manifest must list 20 independent layer pages")
        ...
        if dashboard.count("layers/layer_") != 20:
            raise ValueError(f"{slug}: algorithm dashboard must link 20 layer pages")
        layer_dir = method_dir / "layers"
        expected_layer_files = {
            f"layer_{layer:02d}.html" for layer in range(20)
        }
        ...
        for layer in range(20):
            ...
            if slug == "probeep" or require_complete_raw:
                ...
                required_rail_fields = {
                    ...
                    "dispatch_compute_kind", "dispatch_compute_name",
                    "microbatch", "physical_nic", "subrail",
                    "rail_bandwidth_gbps", "physical_nic_bandwidth_gbps",
                    "dispatch_units", "dispatch_unit_name",
                    "dispatch_bytes_per_unit", "traffic_source",
                    "dispatch_bytes", "weight_bytes", "tx_bytes", "rx_bytes",
                }
                if not required_rail_fields.issubset(
                    csv_fields(data_dir / "rdma_path_load.csv")
                ):
                    raise ValueError(

...
本目录只服务 Test 02，不被其他 Test 调用。`bash/run.sh` 通过这里完成 workload 构造、
...
只放五个算法入口；进入算法后是 58 层目录，再点击进入单独 Layer 页面。单层页面只包含
...
起止时间。DAG 固定表达 `A0 → (A1 ∥ W+D0) → (E0 ∥ W+D1) → E1`，并分开表示同 stream 顺序、event wait、host 完成边界与跨层同类 feedback；实测时间线直接使用 `start_ms/end_ms`，不从 phase 时长拼接；
...
ProbeEP controller 融合在下一层 `balanced_dispatch` 内，不伪造独立 timeline 阶段。本层
...
`validate_run_layout.py --kind test` 对这些文件 fail-closed：时间线 380,480 行、rail 92,800
行、A/M observation 18,240 行，并要求五算法、两个 microbatch、58 层都完整。下一次 H20
run 的 nsys formal pipeline 固定 profile Layer 00 的五个算法；所有 target 使用
`cuda,nvtx,osrt`，不再给 UltraEP 生成空 trace。

```bash
python3 tests/Test_02_RawData1_All58/scripts/reprocess_layers_by_algorithm.py \
  test_logs/<Test02_run_dir>
```

...
def test_all58_plan_is_exact(tmp_path: Path) -> None:
    layer_ids = list(range(58))
    manifest = make_manifest(tmp_path, "raw_data1_all", layer_ids)
    plan = benchmark_plan.build_plan(
        manifest,
        expected_selector="raw_data1_all",

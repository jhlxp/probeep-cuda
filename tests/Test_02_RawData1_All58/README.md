# Test 02：RawData1 All58 H20 多机完整测试

## 1. 测试身份与边界

本目录是一个完整、独立、可单独搬运的多机测试。唯一入口是 `bash/run.sh`；一次入口
只生成一个 Test 02 顶层日志。环境准备、正确性、动态探测 consumer、五算法 benchmark、
Nsight Systems 和可视化都是该日志的内部阶段，不拆成其他 Test 或平级 run。

| 项 | 固定值 |
|---|---|
| 测试目的 | 完整 58 层 DSV3 五算法论文实验 |
| workload selector | `raw_data1_all` |
| MoE layer | 0–57，共 58 层 |
| 论文资格 | 是；全部门禁通过后作为完整 DSV3 主结果 |
| 默认 warmup | 每层 10 次 |
| 默认 measure | 每层 10 次 |
| 默认 repeat | 1 次 |
| 日志 | 一个 `test_logs/run_*_test02_rawdata1_all58_*r/` |
...
| `benchmark.py` | 固定 all58 selector 和 0–57 层，生成不可变 case plan |
...
| `scripts/reprocess_layers_by_algorithm.py` | 从正式 raw 数据生成“算法入口→58 个独立 layer”的离线 HTML/CSV/JSON ZIP；只在同层十轮内统计 |
...
| tokens/rank | 4096 | 正式 all58 测量固定 |
...
比例。本测试只选择 Layer 0–57，并按当前 world size 缩放每层总量：
...
| 必查项 | PASS |
|---|---|
| selector | 恰为 `raw_data1_all` |
| selected layers | 恰为 `[0,1,...,57]` |
| experts | 256 |
...
| case count | 默认 `58 layers × 5 methods × 1 repeat = 290` |

这是完整 workload。报告仍按 Layer 独立展示；只有 Test 02 全部正确性、raw、nsys 与目录
门禁通过后，才允许另行计算 58 层汇总指标。
...
```bash
bash -n \
  tests/Test_02_RawData1_All58/bash/run.sh \
  tests/Test_02_RawData1_All58/scripts/create_venv.sh \
  tests/Test_02_RawData1_All58/scripts/launch_node.sh \
  tests/Test_02_RawData1_All58/scripts/slurm_job.sh \
  tests/Test_02_RawData1_All58/scripts/launch_slurm.sh

python3 -m pytest -q \
  tests/Test_02_RawData1_All58/scripts/test_benchmark_plan.py \
  tests/Test_02_RawData1_All58/scripts/test_raw_data1.py \
  tests/Test_02_RawData1_All58/scripts/test_execution_system.py
...
| 顺序 | 日志阶段 | PASS 条件 |
|---:|---|---|
| 0 | `workload/` | 58 层 manifest、routes 和 case plan 固定 |
...
| 9 | `benchmark/` | 五算法、58 层、同 route、dual-microbatch HT 完成 |
...
```text
test_logs/run_<UTC>_test02_rawdata1_all58_<world>r/
├── workload/
│   ├── manifest.json
│   ├── benchmark_plan.json
│   └── layer_00_topk_idx.npy ... layer_57_topk_idx.npy
├── setup/
│   ├── preflight/
│   ├── build/
│   └── import/
...
│   └── formal-pipeline/
└── artifacts/
    ├── raw_data1_layers_00_57_by_algorithm_rounds_11_20_mean.zip
    └── raw_data1_layers_00_57_by_algorithm_rounds_11_20_mean/
...
| `artifacts/.../manifest.json` | 只记录五算法与 58 个独立 Layer 页面，不保存跨层性能汇总 |
...
下一次正式 Test 02 的固定 raw 行数如下；少一行即使 benchmark status 为 PASS，也不能生成
可引用报告：

| 文件 | 固定行数 | 计算 |
|---|---:|---|
| `iterations.csv` | 2,900 | 290 cases × 10 measured rounds |
| `rank_samples.csv` | 46,400 | 290 × 10 × 16 ranks |
| `microbatch_rank_samples.csv` | 92,800 | 290 × 10 × 16 × 2 MB |
| `microbatch_timeline.csv` | 380,480 | 四 baseline × 58 layers × 10 × 16 × 8 stages + ProbeEP × 58 × 10 × 16 × 9 stages |
| `rdma_path_load.csv` | 92,800 | 290 × 10 × 2 MB × 16 directed rails；当前为 2 servers |
| `probeep_observation_samples.csv` | 18,240 | 57 consuming layers × 10 × A/M × 16 ranks；Layer 00 只有 bootstrap、没有伪 observation |
| `probeep_plan_summary.jsonl` | 1,160 | 58 ProbeEP layers × 10 × A/M plans |
...
`artifacts/raw_data1_layers_00_57_by_algorithm_rounds_11_20_mean.zip` 必须完整镜像同名目录，
且无 CDN/外链脚本。根页只提供五个算法入口；每个算法入口进入 58 层目录，Layer 00–57
...
| 页面数据 | fresh run 门禁 |
|---|---|
| strict timeline | 五算法、58 层、10 rounds、16 ranks 均有真实 CUDA-event interval |
...
|---|---|---|---|
| 本测试静态合同 | PASS | local CPU static tests | 16/16 PASS；含新 logger、五算法 nsys 和 fail-closed raw 门禁 |
...
| all58 five-algorithm benchmark |  |  |  |
| nsys overlap |  |  |  |
| visualization ZIP |  |  |  |
| 顶层 Test log 验收 |  |  |  |
